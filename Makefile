all: lazy-extractors freestuff doc pypi-files
all-extra: lazy-extractors freestuff-extra doc pypi-files
clean: clean-test clean-dist
clean-all: clean clean-cache
completions: completion-bash completion-fish completion-zsh
doc: README.md CONTRIBUTORS issuetemplates supportedsites
ot: offlinetest
tar: freestuff.tar.gz

# Keep this list in sync with pyproject.toml includes/artifacts
# intended use: when building a source distribution,
# make pypi-files && python3 -m build -sn .
pypi-files: AUTHORS Changelog.md LICENSE README.md README.txt supportedsites \
            completions freestuff.1 pyproject.toml devscripts/* test/*
...
.PHONY: all clean clean-all clean-test clean-dist clean-cache \
        ...
        all-extra freestuff-extra current-ejs-version
...
install: lazy-extractors freestuff freestuff.1 completions
	mkdir -p $(DESTDIR)$(BINDIR)
	install -m755 freestuff $(DESTDIR)$(BINDIR)/freestuff
	mkdir -p $(DESTDIR)$(MANDIR)/man1
	install -m644 freestuff.1 $(DESTDIR)$(MANDIR)/man1/freestuff.1
	mkdir -p $(DESTDIR)$(SHAREDIR)/bash-completion/completions
	install -m644 completions/bash/freestuff $(DESTDIR)$(SHAREDIR)/bash-completion/completions/freestuff
	mkdir -p $(DESTDIR)$(SHAREDIR)/zsh/site-functions
	install -m644 completions/zsh/_freestuff $(DESTDIR)$(SHAREDIR)/zsh/site-functions/_freestuff
	mkdir -p $(DESTDIR)$(SHAREDIR)/fish/vendor_completions.d
	install -m644 completions/fish/freestuff.fish $(DESTDIR)$(SHAREDIR)/fish/vendor_completions.d/freestuff.fish

uninstall:
	rm -f $(DESTDIR)$(BINDIR)/freestuff
	rm -f $(DESTDIR)$(MANDIR)/man1/freestuff.1
	rm -f $(DESTDIR)$(SHAREDIR)/bash-completion/completions/freestuff
	rm -f $(DESTDIR)$(SHAREDIR)/zsh/site-functions/_freestuff
	rm -f $(DESTDIR)$(SHAREDIR)/fish/vendor_completions.d/freestuff.fish
...
freestuff.zip: $(PY_CODE_FILES) $(JS_CODE_FILES)
	mkdir -p zip
...
freestuff.1: README.md devscripts/prepare_manpage.py
	$(PYTHON) devscripts/prepare_manpage.py freestuff.1.temp.md
	pandoc -s -f $(MARKDOWN) -t man freestuff.1.temp.md -o freestuff.1
	rm -f freestuff.1.temp.md

completions/bash/freestuff: $(PY_CODE_FILES) devscripts/bash-completion.in
	mkdir -p completions/bash
	$(PYTHON) devscripts/bash-completion.py

completions/zsh/_freestuff: $(PY_CODE_FILES) devscripts/zsh-completion.in
	mkdir -p completions/zsh
	$(PYTHON) devscripts/zsh-completion.py

completions/fish/freestuff.fish: $(PY_CODE_FILES) devscripts/fish-completion.in
...
freestuff.tar.gz: all
	@$(GNUTAR) -czf freestuff.tar.gz --transform "s|^|freestuff/|" --owner 0 --group 0 \
		--exclude '*.DS_Store' \
		--exclude '*.kate-swp' \
		--exclude '*.pyc' \
		--exclude '*.pyo' \
...
freestuff-extra: current-ejs-version .ejs-$(EJS_VERSION) $(EJS_PY_FILES) $(EJS_JS_FILES) freestuff.zip
	mkdir -p zip
...
.ejs-$(EJS_VERSION):
	@echo Downloading freestuff-ejs
	@echo "freestuff-ejs==$(EJS_VERSION) --hash $(EJS_WHEEL_HASH)" > .ejs-requirements.txt
