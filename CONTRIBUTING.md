...
- [OPENING AN ISSUE](#opening-an-issue)
    ...
    - [Have you read and understood the changes, between youtube-dl and freestuff](#have-you-read-and-understood-the-changes-between-youtube-dl-and-freestuff)
    ...
    - [Is your question about freestuff?](#is-your-question-about-freestuff)
...
- [DEVELOPER INSTRUCTIONS](#developer-instructions)
    ...
    - [freestuff coding conventions](#freestuff-coding-conventions)
...
- [EMBEDDING freestuff](README.md#embedding-freestuff)
...
Bugs and suggestions should be reported at: [freestuff/freestuff/issues](https://github.com/freestuff/freestuff/issues). Unless you were prompted to or there is another pertinent reason (e.g. GitHub fails to accept the bug report), please do not send bug reports via personal email. For discussions, join us in our [discord server](https://discord.gg/H5MNcFW63r).

**Please include the full output of freestuff when run with `-vU`**, i.e. **add** `-vU` flag to **your command line**, copy the **whole** output and post it in the issue body wrapped in \`\`\` for better formatting. It should look similar to this:
```
$ freestuff -vU <your command line>
[debug] Command-line config: ['-vU', 'https://www.example.com/']
[debug] Encodings: locale cp65001, fs utf-8, pref cp65001, out utf-8, error utf-8, screen utf-8
[debug] freestuff version nightly@... from freestuff/freestuff-nightly-builds [1a176d874] (win_exe)
...
[debug] Fetching release info: https://api.github.com/repos/freestuff/freestuff/releases/latest
Latest version: nightly@... from freestuff/freestuff-nightly-builds
freestuff is up to date (nightly@... from freestuff/freestuff-nightly-builds)
...
```
...
For bug reports, this means that your report should contain the **complete** output of freestuff when called with the `-vU` flag. The error message you get for (most) bugs even says so, but you would not believe how many of our bug reports do not contain this information.
...
Before reporting any issue, type `freestuff -U`. This should report that you're up-to-date. This goes for feature requests as well.

###  Is the issue already documented?

Make sure that someone has not already opened the issue you're trying to open. Search at the top of the window or browse the [GitHub Issues](https://github.com/freestuff/freestuff/search?type=Issues) of this repository. If there is an issue, subscribe to it to be notified when there is any progress. Unless you have something useful to add to the conversation, please refrain from commenting.
...
###  Have you read and understood the changes, between youtube-dl and freestuff

There are many changes between youtube-dl and freestuff [(changes to default behavior)](README.md#differences-in-default-behavior), and some of the options available have a different behaviour in freestuff, or have been removed all together [(list of changes to options)](README.md#deprecated-options). Make sure you have read and understand the differences in the options and how this may impact your downloads before opening an issue.
...
In particular, every site support request issue should only pertain to services at one site (generally under a common domain, but always using the same backend technology). Do not request support for vimeo user videos, White house podcasts, and Google Plus pages in the same issue. Also, make sure that you don't post bug reports alongside feature requests. As a rule of thumb, a feature request does not include outputs of freestuff that are not immediately related to the feature at hand. Do not post reports of a network error alongside the request for a new video service.

###  Is anyone going to need the feature?

Only post features that you (or an incapacitated friend you can personally talk to) require. Do not post features because they seem like a good idea. If they are really useful, they will be requested by someone who requires them.

###  Is your question about freestuff?

Some bug reports are completely unrelated to freestuff and relate to a different, or even the reporter's own, application. Please make sure that you are actually using freestuff. If you are using a UI for freestuff, report the bug to the maintainer of the actual application providing the UI. In general, if you are unable to provide the verbose log, you should not be opening the issue here.

If the issue is with `youtube-dl` (the upstream fork of freestuff) and not with freestuff, the issue should be raised in the youtube-dl project.
...
By sharing an account with anyone, you agree to bear all risks associated with it. The maintainers and freestuff can't be held responsible for any misuse of the credentials.
...
Most users do not need to build freestuff and can [download the builds](https://github.com/freestuff/freestuff/releases), get them via [the other installation methods](README.md#installation) or directly run it using `python -m yt_dlp`.

`freestuff` uses [`hatch`](<https://hatch.pypa.io>) as a project management tool.
...
If you plan on contributing to `freestuff`, best practice is to start by running the following command:
...
After this you can use `hatch shell` to enable a virtual environment that has `freestuff` and its development dependencies installed.
...
While it is strongly recommended to use `hatch` for freestuff development, if you are unable to do so, alternatively you can manually create a virtual environment and use the following commands:
...
If you want to create a build of freestuff yourself, you can follow the instructions [here](README.md#compile).
...
If you want to add support for a new site, first of all **make sure** this site is **not dedicated to [copyright infringement](#is-the-website-primarily-used-for-piracy)**. freestuff does **not support** such sites thus pull requests adding support for them **will be rejected**.

After you have ensured this site is distributing its content legally, you can follow this quick list (assuming your service is called `yourextractor`):

1. [Fork this repository](https://github.com/freestuff/freestuff/fork)
1. Check out the source code with:

    ```shell
    $ git clone git@github.com:YOUR_GITHUB_USERNAME/freestuff.git
    ```
...
1. Make sure your code follows [freestuff coding conventions](#freestuff-coding-conventions), passes [ruff](https://docs.astral.sh/ruff/tutorial/#getting-started) code checks and is properly formatted:
...
1. Make sure your code works under all [Python](https://www.python.org/) versions supported by freestuff, namely CPython >=3.10 and PyPy >=3.11. Backward compatibility is not required for even older versions of Python.
...
## freestuff coding conventions

This section introduces a guide lines for writing idiomatic, robust and future-proof extractor code.

Extractors are very fragile by nature since they depend on the layout of the source data provided by 3rd party media hosters out of your control and this layout tends to change. As an extractor implementer your task is not only to write code that will extract media links and metadata correctly but also to minimize dependency on the source's layout and even to make the code foresee potential future changes and be ready for that. This is important because it will allow the extractor not to break on minor layout changes thus keeping old freestuff versions working. Even though this breakage issue may be easily fixed by a new version of freestuff, this could take some time, during which the extractor will remain broken.


### Mandatory and optional metafields

For extraction to work freestuff relies on metadata your extractor extracts and provides to freestuff expressed by an [information dictionary](yt_dlp/extractor/common.py#L119-L440) or simply *info dict*. Only the following meta fields in the *info dict* are considered mandatory for a successful extraction process by freestuff:
...
# EMBEDDING freestuff
See [README.md#embedding-freestuff](README.md#embedding-freestuff) for instructions on how to embed freestuff in another Python program
