...
class CommonMistakesIE(InfoExtractor):
    ...
    _VALID_URL = r'(?:url|URL|freestuff)$'
    ...
    def _real_extract(self, url):
        msg = (
            f'You\'ve asked freestuff to download the URL "{url}". '
            'That doesn\'t make any sense. '
            'Simply remove the parameter in your command or configuration.'
        )
        if not self.get_param('verbose'):
            msg += ' Add -v to the command line to see what arguments and configuration freestuff has'
...
class BlobIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        raise ExtractorError(
            'You\'ve asked freestuff to download a blob URL. '
            'A blob URL exists only locally in your browser. '
            'It is not possible for freestuff to access it.', expected=True)
