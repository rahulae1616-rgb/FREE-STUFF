...
Refer to the [PO Token Guide](https://github.com/freestuff/freestuff/wiki/PO-Token-Guide) for more information on PO Tokens.

> [!TIP]
> If publishing a PO Token Provider plugin to GitHub, add the [freestuff-pot-provider](https://github.com/topics/freestuff-pot-provider) topic to your repository to help users find it.
...
> If you are using these APIs outside freestuff plugins, please account for this by importing them safely.
...
class MyPoTokenProviderPTP(PoTokenProvider):  # Provider class name must end with "PTP"
    ...
    def _real_request_pot(self, request: PoTokenRequest) -> PoTokenResponse:
        ...
        # ℹ️ Settings are pulled from extractor args passed to freestuff with the key `youtubepot-<PROVIDER_KEY>`.
        ...
        # as it will handle cookies and other networking settings passed to freestuff.
        try:
        ...
        return PoTokenResponse(
            ...
            # By default, freestuff will use the default ttl from a registered cache spec (see below)
...
> [!IMPORTANT]
> freestuff currently has a built-in LRU Memory Cache Provider and a cache spec provider for WebPO Tokens. 
...
# VERY IMPORTANT: freestuff has a built-in memory cache with a priority of 10000. 
