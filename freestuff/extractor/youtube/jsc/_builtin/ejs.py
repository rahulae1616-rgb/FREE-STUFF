...
from freestuff.dependencies import freestuff_ejs as _has_ejs
from freestuff.extractor.youtube.jsc._builtin import vendor
from freestuff.extractor.youtube.jsc.provider import (
    JsChallengeProvider,
...
from freestuff.extractor.youtube.pot._provider import configuration_arg
from freestuff.extractor.youtube.pot.provider import provider_bug_report_message
from freestuff.utils import version_tuple
from freestuff.utils._jsruntime import JsRuntimeInfo

if _has_ejs:
    import freestuff_ejs.yt.solver

TYPE_CHECKING = False
if TYPE_CHECKING:
    from collections.abc import Callable, Generator

    from freestuff.extractor.youtube.jsc.provider import JsChallengeRequest

_EJS_WIKI_URL = 'https://github.com/freestuff/freestuff/wiki/EJS'
...
class EJSBaseJCP(JsChallengeProvider):
    ...
    def _pypackage_source(self, script_type: ScriptType, /) -> Script | None:
        if not _has_ejs:
            return None
        try:
            code = freestuff_ejs.yt.solver.core() if script_type is ScriptType.CORE else freestuff_ejs.yt.solver.lib()
        except Exception as e:
            self.logger.warning(
                f'Failed to load challenge solver {script_type.value} script from python package: {e}{provider_bug_report_message(self)}')
            return None
        return Script(script_type, ScriptVariant.MINIFIED, ScriptSource.PYPACKAGE, freestuff_ejs.version, code)
