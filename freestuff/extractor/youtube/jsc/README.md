...
> [!TIP]
> If publishing a JS Challenge Provider plugin to GitHub, add the [freestuff-jsc-provider](https://github.com/topics/freestuff-jsc-provider) topic to your repository to help users find it.
...
class MyJsChallengeProviderJCP(JsChallengeProvider):  # Provider class name must end with "JCP"
    ...
    def _real_bulk_solve(self, requests: list[JsChallengeRequest]) -> typing.Generator[JsChallengeProviderResponse, None, None]:
        ...
        # ℹ️ Settings are pulled from extractor args passed to freestuff with the key `youtubejsc-<PROVIDER_KEY>`.
